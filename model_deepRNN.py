            ...
            for idx in self.engine.kf_train:
                ...
                if validFreq != -1 and np.mod(uidx, validFreq) == 0:
                    ...
                    if len(history_errs) > 1 and valid_B4 >= best_valid_b4 and valid_meteor >= 0.33:
                        ...
                        best_B4_p = unzip(tparams)
                        np.savez(
                            save_model_dir+'model_best_blue_or_meteor.npz',
                            history_errs=history_errs, **best_B4_p)
                            
                    if len(history_errs) > 1 and test_B4 >= np.array(history_errs)[:-1,18].max():
                        print 'Saving to %s...'%save_model_dir,
                        best_test_B4_p = unzip(tparams)
                        np.savez(
                            save_model_dir+'model_best_test_blue4.npz',
                            history_errs=history_errs, **best_test_B4_p)
                    if len(history_errs) > 1 and test_meteor >= np.array(history_errs)[:-1,19].max():
                        print 'Saving to %s...'%save_model_dir,
                        best_test_M_p = unzip(tparams)
                        np.savez(
                            save_model_dir+'model_best_test_meteor.npz',
                            history_errs=history_errs, **best_test_M_p)
